...
@section('content')
                ...
                <div>
                    ...
                    <table class="w-full border-2 border-gray-600 text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3"style="max-width: 100px;">Estado</th>
                        ...
                        <tbody class="abx acb">
                            @foreach ($solicitudes as $solicitud)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-b-slate-50">
                                    ...
                                    <td class="px-6 py-4" style="max-width: 100px;">{{ $Solicitud->usuario->nombre }}</td>
                                    <td class="px-6 py-4" style="max-width: 100px;">{{ $Solicitud->equipos->nombre }}</td>
                                    <td class="px-6 py-4" style="max-width: 100px;">{{ $Solicitud->estado }}</td>
                                    <td class="px-6 py-4">
                                        @foreach ($solicitud->equipos as $equipo)
                                            {{ $equipo->nombre }}: {{ $equipo->valor }}<br>
                                        @endforeach
                                    </td>
                                    ...
                                    <td class="px-6 py-4" style="max-width: 200px;">
                                        <a class="px-3 mx-3 font-medium text-blue-600 dark:text-blue-500 hover:underline"
                                            href="{{ route('solicitud.edit', $solicitud->id) }}">Editar</a>
                    ...
                    <div class="mt-3" style="display: flex; justify-content: center;">
                        <button
                            ...
                            <a href="{{ route('solicitudes.create') }}">Crear solicitud</a>
                        </button>
                    </div>
                </div>
            </div>
    ...
    <script>
        function borrar(id) {
            console.log('¿Está seguro de eliminar la solicitud?', id);
            Swal.fire({
                title: 'Confirmación',
        ...
        filtroNombreInput.addEventListener('input', function() {
            ...
            filas.forEach(function(filaSolicitud) {
                ...
                if (nombreSolicitud.includes(filtro)) {
